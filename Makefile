...
# KubeRocketCI/tekton-custom-task-bundle:$VERSION and KubeRocketCI/tekton-custom-task-catalog:$VERSION.
IMAGE_TAG_BASE ?= KubeRocketCI/tekton-custom-task
...
test: envtest ## Run tests.
	KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) -p path)" go test ./... -coverprofile=coverage.out `go list ./...`

.PHONY: lint-fix
lint: golangci-lint ## Run golangci-lint linter and perform fixes
...
GOBIN=$(LOCALBIN) GOCACHE=$$GOCACHE GOMODCACHE=$$GOMODCACHE go install $${package} ;\
}
endef
